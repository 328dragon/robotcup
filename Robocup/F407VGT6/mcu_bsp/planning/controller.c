...
void Controller_Init(Controller_t *controller, StepMotorZDT_t **_zdt_motor, Kinematic_t *kinematic)
{
    ...
    // 初始化PID控制器
    PID_struct_init(&controller->pid_x, POSITION_PID, init_x_maxout, 0.5f, 0.8f, 0.04f, 0.04);
    PID_struct_init(&controller->pid_y, POSITION_PID, init_y_maxout, 0.5f, 0.8f, 0.04f, 0.04);
