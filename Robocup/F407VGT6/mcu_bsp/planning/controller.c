...
void Controller_Init(Controller_t *controller, StepMotorZDT_t **_zdt_motor, Kinematic_t *kinematic)
{
    ...
    uint32_t init_x_maxout = 0.6f;
    uint32_t init_y_maxout = 0.6f;
    uint32_t init_yaw_maxout = 1;
    // 初始化PID控制器
    PID_struct_init(&controller->pid_x, POSITION_PID, init_x_maxout, 0.4f, 0.1f, -0.01f, 0);
    PID_struct_init(&controller->pid_y, POSITION_PID, init_y_maxout, 0.4f, 0.1f, -0.01f, 0);
    PID_struct_init(&controller->pid_yaw, POSITION_PID, init_yaw_maxout, 0.6f, 0.1f, -0.01f, 0);
